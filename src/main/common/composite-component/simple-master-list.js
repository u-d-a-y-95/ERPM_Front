...
function SimpleMasterList(props) {
  ...
  const [actionModalStateList, setActionModalStateList] = useState(
    new Array(50)
  );
  const searchKey = useRef(
    props.config.headers
      .filter((item) => item["canSearch"])
      .map((item) => item["key"])
  );
            ...
            <tr key={"_index"}>
              {props.config?.headers?.map((item, index) => {
                return (
                  <th
                    key={index}
                    style={item["hColStyle"] || item["style"]}
                    className={item["hColClass"] || item["class"]}
                  >
                    {item["label"]}
                  </th>
                );
              ...
              {(props.config.action?.length > 0 && (
                <th key={"actionBtnThIndex"} style={{ width: "10px" }}></th>
              )) ||
                null}
            </tr>
          </thead>
                  ...
                  {props.config.headers.map((header, index) => {
                    if (header["isClickAble"]) {
                      return (
                        <td
                          key={index}
                          style={header["bColStyle"] || header["style"]}
                          className={header["bColClass"] || header["class"]}
                          onClick={columClicked.bind(this, listItem, header)}
                        >
                          {item[header["key"]]}
                        </td>
                      );
                  ...
                  {(props.config.action?.length > 0 && (
                    <td style={{ width: "10px" }}>
                      <span className='list-action'>
                        <i
                          className='fa fa-ellipsis-v list-action-i'
                          onClick={actionBtnClicked.bind(this, dataIndex)}
                        ></i>
